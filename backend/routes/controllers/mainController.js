import { SortService, Direction } from "../../deps.js";
import * as mainService from "../../services/mainService.js";

const passwd = Deno.env.get("PASSWORD");
const sortOptions = [
  { fieldName: 'timestamp', direction: Direction.DESCENDING }
]
...
const showMain = async ({ request, render, response }) => {
  if (request.method === "POST") {
    ...
    res.forEach((row) => {
      statData.block.push(row);
    });

    res = await mainService.getLogs();
    statData.logs = JSON.parse(JSON.stringify(res));
    ...
    statData.logs = SortService.sort(statData.logs, sortOptions);
